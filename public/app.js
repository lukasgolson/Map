...
function updateUI() {
  ...
  if (progressBarEl) {
    ...
    const wrapper = progressBarEl.closest('.progress-bar-wrapper');
    if (wrapper) wrapper.setAttribute('aria-valuenow', Math.round(progressPercent));
  }
...
function updateBatteryUI(level) {
  const percentText = document.getElementById('battery-val');
  if (percentText) {
  ...
  if (bar) {
    ...
    const wrapper = bar.closest('.battery-bar-wrapper');
    if (wrapper) wrapper.setAttribute('aria-valuenow', level);
    if (level <= 20) {
